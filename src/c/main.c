...
static void draw_rect(GContext *ctx, int w, int h) {
  ...
  int hticks = max_h - 1;   // 11 or 23
  int mticks = 11;           // :05 :10 ... :55
  ...
  // Hour labels (right-aligned in LABEL_W column left of bar)
  {
  ...
  // Minute labels (left-aligned right of minute bar)
  {
    GFont font = fonts_get_system_font(FONT_KEY_GOTHIC_14);
    graphics_context_set_text_color(ctx, col_txt);
    char buf[4];
